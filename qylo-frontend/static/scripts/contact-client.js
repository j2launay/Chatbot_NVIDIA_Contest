...
function startRivaService() {
    ...
    $("#riva-btn").removeClass("btn-danger").addClass("btn-success")
    ...
    if (websocket == null || websocket.readyState !== WebSocket.OPEN) {
        websocket = new WebSocket('wss://' + "rdi.behit.net/asr/");
        ...
        websocket.addEventListener('open', function (evt) {
            ...
            start_asr = { "type": "start", "language": "en-US", "format": "raw", "encoding": "LINEAR16", "sampleRateHz": 16000 ,"intermidiate":false,
            "sttSpeechContexts":[{
                phrases : ["Polypectomy","Nasopharynx","Delhi","Pepperoni","Florentine","Tropicana","Calzone","Fresco",
                "Provolone","Vinaigrette","Mountain Dew"],
                boost : 200.,
              }]};  
        ...
        websocket.addEventListener('message', function (result) {
            ...
            if (result_data.type === "started") {
                    ...
                    worker.onmessage = function (msg) {
                        if (msg.data.command == 'newBuffer') {
                            if (websocket.readyState === WebSocket.OPEN) {
                                ...
                                //Python web bridge work
                                //
                                //socket.emit('asr_request', {"audio":msg.data.resampled.buffer});
                                //End Python web bridge work
                            }
                        }
                    };
                };
                ...
                document.getElementById('interface').setAttribute('placeholder', 'Enter some text to annotate, or start speaking');
                ...
                if (result_data.type === "recognition") {
                    if(result_data.alternatives[0].text.length>15){
                        document.getElementById('interface').value = result_data.alternatives[0].text;
...
function stopRivaService() {
    ...
    $("#riva-btn").removeClass("btn-success").addClass("btn-danger")
    if (websocket.readyState === WebSocket.OPEN) {
...
$(document).ready(function () {
    ...
    ////Asr python Client dev

    //socket.on("asr_connected",(data)=>{
    //    console.log(data)
    //})

    //End Asr python Client dev
...
$(document).on("click", "#riva-btn", function (e) {
    //socket.emit('asr_connect', {"type": "start", "language": "en-US", "format": "raw", "encoding": "LINEAR16", "sampleRateHz": 16000 ,"intermidiate":false});
    //return ;
    if(!rivaRunning){
        startRivaService();
    }else{
    ...
    rivaRunning=!rivaRunning;
    console.log(rivaRunning)
});
